...
% - Remeasure the leaf spring constant
% - Try new alignment parts
% - Assuming alignment works, put in the figures where required
% - Find the correct clip coating material
% - Finish tetrode-param table (has XX in table and caption).
% - Video of bobbin loading and TT making. This goes on youtube and link in appendix
%
% - Ming and Jakob proof read
% - Bioarxiv
%
% - Tweet:
%   1. Here is my work with @jvoigts, @OpenEphys, and @SpikeGadgets on an open source, cheap, and fast tetrode making machine (link). We have been using it in the Wilson lab for the past 2 years. It allows you to make probes quickly and reliably. Some features: (1/N)
%   2. Feature 1: No wire folding! just draw it from pre-loaded bobbins.
%   3. Feature 2: Auto-aligning, magnetic wire bundle retention mechanism results in perfectly straight probes every time. 
%   4. Feature 3: 3D-printed leaf spring provides constant wire tension during high speed twist
%   5. Feature 4: Turn as fast as you want. We use 1000 RPM. Your tetrode is done in seconds :).
%   6. Put a tick on the sheet (link) if you want one. We are gauging interest for distribution on the Open Ephys store (link).
...
\diameter12.7$\mu$m}, the 2nd stiction setting (corresponding to a threshold of
...
\textbf{Settings}   & Bobbin stiction                   & \multicolumn{2}{c}{Set point 2 ($\sim$11.5 mN)} \\
                    ...
                    & Hot air flow\textsuperscript{1}   & \multicolumn{2}{c}{$\sim$5.8 L/m} \\
                    &                                   & \\
...
\textbf{Results}    & TWP length                        & 37 mm            & 55 mm \\
                    ...
                    & Avg. load time\textsuperscript{2} & \multicolumn{2}{c}{21.1 $\pm$ 10.0 sec} \\
                    & Turn time                         & 8 sec           & 9.5 sec \\
                    & Avg. fuse time\textsuperscript{2} & \multicolumn{2}{c}{11.1 $\pm$ 5.6 sec} \\
                    & Avg. cut/removal time\textsuperscript{2}  & \multicolumn{2}{c}{11.3 $\pm$ 4.5 sec} \\
...
\begin{enumerate}[noitemsep]
    ...
    \item Use the 60 mm M3 screw to mount the bobbin assembly to the standoff
        captive within the feeder base. Repeat for both sides.
...
\begin{enumerate}[noitemsep]
    \item Remove the long, M3 step screws from the bottom of the stepper motor
        (4x).
    \item Use 40mm M3 screws (4x) to attach the motor mount to the bottom of
        the motor.
    \item Press fit two magnets into the spring rotor top \textit{in the same
        orientation}. Make sure they are pushed down until below the plastic
        surface so that they don't interfere with the flat mating surface of
        the piece.
    \item Stick two \textit{additional} magnets to those you just inserted into
        spring rotor and then use the spring rotor to press fit these two
        magnets into the spring rotor base. This procedure ensures that magnets
        will be press fit into the spring rotor base with the correct polarity.
        Again, make sure the magnets are pressed until below the flat,
        3D-printed surface.
    \item Push the spring rotor base, now with captive magnets, onto the
        stepper motor shaft.
