...
\caption[]{Bobbin loading and TWP creation procedures. 

\textbf{(A) Summary of the bobbin loading procedure. See text for details.}
\begin{enumerate}[noitemsep]
    \item The leaf spring is removed and a bobbin is magnetically mated with
        the rotor. The wire guide is moved into close proximity to the wire
        groove on the bobbin.
    \item Wire from the stock spool is threaded through the wire guide and
        around the bobbin. The control box is then used to load a desired
        amount of stock wire.
\end{enumerate}

\textbf{(B) Summary of the TWP construction procedure. See text for details.}
\begin{enumerate}[noitemsep]
    \item The leaf spring is attached to the base rotor and the wire guide is
        moved out of the way.
    \item The loose wire is alligator clipped and drawn from the bobbins. 
    \item The user's free hand is used pull up on the twisting attachment's
        leaf spring until under slight tension.
    \item The alligator clip is drawn down and flipped 180$^{\circ}$ to meet
        the magnet within the alignment jig (insets). This procedure improves
        the alligator clip's grip on the bundle. A twist can then be performed.
    \item After the twist, wires are fused \textit{starting from point at which they separate
        towards the bobbins} using a hot air gun.
    \item The bobbins are simultaneously rolled forward forward to release
        tension on the wire.
    \item The loose wire is cut e above the point at which the wires are fused,
        and the alligator clip is removed from the magnet with a finished TWP.
    \item The finished TWP is cut into a storage box.
\end{enumerate}
}
\label{f:usage}
\end{figure}
...
The following steps detail TWP construction using Twister3. If you are
interested in making stereotrodes instead of tetrodes, follow the same steps
but only use two bobbins on diagonally opposing sides of the feeder. A video of
the TWP construction process is available to aid these instructions
(Appendix~\ref{s:videos}).
...
in Table~\ref{t:mech_bom}. In addition to these standard mechanical components,
several 3D-printed parts are required (Table~\ref{t:3d_bom}). These parts are
...
steps detailed Figures~\ref{f:feeder_assem},~\ref{f:other_assem}
(A),~\ref{f:other_assem} (B), and~\ref{f:other_assem} (C), respectively. After
each of these modules is complete, they are combined into the complete device
by following the steps presented in Figure~\ref{f:twister3_assem}.
...
\begin{enumerate}[noitemsep]
    ...
    \item Push each of the bearing cases into the stock spool of microwire.
...
\begin{enumerate}[noitemsep]
    ...
    \item With the wire-clip mechanism installed, slide the adjustment plate
        around until the motor axis of rotation (dotted black line) is precisely
